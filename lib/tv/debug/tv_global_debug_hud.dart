...
/// Lightweight global TV debug overlay.
///
/// Keep this on during development. It does not write files, does not collect
/// frame timings, and does not consume remote events.
...
class _TvGlobalDebugHudState extends State<TvGlobalDebugHud> {
  // Temporary diagnostic flag. Keep HUD visible in CI release/profile APKs.
  // Remove or set false only before final release.
  ...
  Widget _hud(BuildContext context) {
    ...
    final lines = <String>[
      'LIVEGO LITE DEBUG HUD',
      'up=${seconds}s keys=$_keyCount last=$_lastEvent/$_lastKey',
      'focus=$_primaryFocus',
      'route=$_route',
      'screen=$sizeText',
      'record=OFF heavy-perf=OFF',
      'mode=global-root lite no-consume',
