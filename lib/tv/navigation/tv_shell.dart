...
class _TvShellState extends ConsumerState<TvShell> {
  ...
  // Home stays warm for main landing/focus memory. Account/Search/Library/
  // Downloads are disposable so inactive screens do not keep running.
  ...
  bool _backSuppressedNow() {
    return DateTime.now().millisecondsSinceEpoch < _suppressBackUntilMs;
  }

  void _holdRootFocusDuringBackHandoff() {
  ...
  KeyEventResult _exitKey(FocusNode node, KeyEvent event) {
    ...
    if (tvIsBackKey(key)) {
      if (_backSuppressedNow()) return KeyEventResult.handled;
      _closeExit();
      return KeyEventResult.handled;
    }
    return KeyEventResult.ignored;
